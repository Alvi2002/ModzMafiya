  ...
  return (
    <div className="p-4 pb-28">
      <div className="relative mt-2">
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search Apps, Presets, Prompts..." className="w-full bg-cardBg border border-borderGray rounded-2xl py-4 px-5 focus:border-primary text-gray-300 outline-none" />
  ...
  return (
    <Router>
      <div className={isDark ? "dark" : "light"}>
        <div className={`min-h-screen transition-all ${isDark ? "bg-darkBg text-white" : "bg-gray-100 text-black"}`}>
          ...
          <div className={`fixed inset-y-0 right-0 w-64 ${isDark ? "bg-[#161b29]" : "bg-white"} z-[70] transform ${isMenuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 border-l border-borderGray p-6`}>
             <button onClick={() => setIsMenuOpen(false)} className="mb-8 p-2 bg-darkBg rounded-full text-white"><X size={20}/></button>
